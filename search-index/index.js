...
import { zeroFillEntryId } from "./helpers.js";
...
async function index(id, title, entry_seo, comment, context, xrt, mtype) {
  var zerofilled = zeroFillEntryId(id);

  const machineType = mtype ? (mtype === null ? "" : mtype) : "";
  /**
   * INPUT:
   * 1) Full title
  ...
  var inputs = [
    {
      input: title,
      contexts: { genre: context, machineType: machineType, xrated: xrt, genre_xrated: [`${context}_${xrt}`, `ALL_${xrt}`] },
      weight: 20,
    },
  ];
  ...
  // console.log(JSON.stringify(esObject, null, 2));
  if (true) {
    await client.index(esObject);
  }
}

async function getTitles(query, context, connection) {
  // Wrap mysql callback API so callers can await completion before shutting down the pool.
    ...
    connection.query(query, [], async function (error, results, fields) {
      ...
      try {
        ...
        for (; i < results.length; i++) {
          console.log(`[${zeroFillEntryId(results[i].id)}][FOUND] ${context} - ${results[i].machine_type_text} - ${results[i].full_title}(xrt: ${results[i].xrated})`);
          var comment = "";
          var entry_seo = "";
          var xrt = false;
          ...
          switch (context) {
            case "SOFTWARE":
              ...
              console.log(`[${zeroFillEntryId(results[i].id)}][FOUND] ${context} - ${results[i].machine_type_text} - ${results[i].full_title}(xrt: ${results[i].xrated})`);
              if (results[i].machine_type_text === null) {
                console.error(`[${zeroFillEntryId(results[i].id)}][ERROR][SOFTWARE] machineType is MISSING - ${results[i].full_title}`);
                console.log(`[${zeroFillEntryId(results[i].id)}][ERROR][SOFTWARE] machineType is MISSING - ${results[i].full_title}`);
              }
            ...
            case "HARDWARE":
              if (results[i].machine_type_text === null) {
                console.error(`[${zeroFillEntryId(results[i].id)}][WARNING][HARDWARE] machineType is MISSING - ${results[i].full_title}`);
                console.log(`[${zeroFillEntryId(results[i].id)}][WARNING][HARDWARE] machineType is MISSING - ${results[i].full_title}`);
              }
            case "BOOK":
              entry_seo = results[i].entry_seo;
              if (results[i].pub_names) {
          ...
          await index(results[i].id, results[i].full_title.replace(/\[MOD\] /g, ""), entry_seo, comment, context, xrt, results[i].machine_type_text);

          // if entry got alias entries, add them as well
          if (results[i].aliases) {
            const aliases = results[i].aliases.split(";;");
            console.log(`[${zeroFillEntryId(results[i].id)}][ALIAS] Found alias - (${aliases.length})`);
            for (let value of aliases) {
              const aliastitle = `${value} (${results[i].full_title.replace(/\[MOD\] /g, "")})`;
              console.log(`[${zeroFillEntryId(results[i].id)}][ALIAS] ${aliastitle}`);
              await index(results[i].id, aliastitle, entry_seo, comment, context, xrt, results[i].machine_type_text);
            }
          } else {
            console.log(`[${zeroFillEntryId(results[i].id)}][ALIAS] NO Alias found for: ${results[i].full_title}`);
          }
        }
        resolve();
      } catch (e) {
...
async function run() {
  ...
  try {
    await getTitles(softwareSQL, "SOFTWARE", connection);
    await getTitles(hardwareSQL, "HARDWARE", connection);
    await getTitles(booksSQL, "BOOK", connection);
    await getTitles(magazinesSQL, "MAGAZINE", connection);
    await getTitles(entitiesSQL, "ENTITY", connection);
    await getTitles(groupsSQL, "GROUP", connection);
    await getTitles(licensesSQL, "LICENSE", connection);
  } finally {

...
elif [ $TO_EXPORT == 'search' ]
then
    echo "EXPORT zxinfo-search"
...
if [ $TO_EXPORT == 'all' ]||[ $TO_EXPORT == 'search' ]
then
    # clean-up
    rm -f zxinfo-search.index.txt zxinfo-search.mappings.txt

    echo "DUMPING zxinfo-search DATA"
    $DUMP_CMD \
      --input=http://localhost:9200/zxinfo-search \
      --output=zxinfo-search.index.txt \
      --type=data \
      --headers='{"Content-Type": "application/json"}'

    echo "DUMPING zxinfo-search MAPPINGS"
    $DUMP_CMD \
      --input=http://localhost:9200/zxinfo-search \
      --output=zxinfo-search.mappings.txt \
      --type=mapping \
      --headers='{"Content-Type": "application/json"}'
